/**
 * Shared types - directly ported from `apps/web/src/lib/types.ts`.
 *
 * This is a 1:1 copy of the web app's client-side type definitions so that
 * hooks, API calls, and components in the mobile app share the same shape.
 * All types are pure TypeScript (no runtime deps) so they work identically

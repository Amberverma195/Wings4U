...
function CardsContent() {
  ...
  const [cards, setCards] = useState<Array<{ id: string; last4: string }>>([]);
  ...
  const handleAddCard = (e: React.FormEvent) => {
    ...
    setCards((current) => [
      ...current,
      { id: Date.now().toString(), last4: cardNumber.slice(-4) || cardNumber },
    ]);
    ...
    <div className={styles.pageShell}>
      ...
      {showAddModal && (
        <div className="wk-method-overlay" onMouseDown={() => setShowAddModal(false)}>
          <div className="wk-method-card" onMouseDown={(e) => e.stopPropagation()}>
            <div
